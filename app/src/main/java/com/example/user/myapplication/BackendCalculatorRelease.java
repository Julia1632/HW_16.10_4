...
public class BackendCalculatorRelease implements ICalculator {
    String base_url=Constants.BASE_RELEASE_URL;
    @Override
    ...
    public String mult(int a,int b){
        final String url = new CalculatorMethods(base_url).calculateMult(a,b);
        final MyResponseListener listener = new MyResponseListener();
        new HttpClient().request(url, listener);
        return String.valueOf(listener.getResult().getSum());
    }
